...
function DataHandler (opts, primusOpts) {
  ...
  this.__handleGraphQLError = null
  // models
...
DataHandler.prototype._handleQueryOrMutation = function (payload) {
  ...
  return this._queryExecutor.execute(payload).then(function (resPayload) {
    ...
    self._handleQueryError(payload, err)
  })
}

DataHandler.prototype._handleQueryError = function (payload, err) {
  // check if spark has disconnected
  // this line is covered... not sure what is going on
  /* istanbul ignore next */
  if (!this._spark) return
  this._responder.sendErrs(payload.id, err.status, [err])
  if (this.__handleGraphQLError) {
    this.__handleGraphQLError(err, payload)
  }
}

/**
 * attach graphql error listener
 */
DataHandler.prototype.onGraphQLError = function (handler) {
  this.__handleGraphQLError = handler
}
