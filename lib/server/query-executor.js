...
var checkForPayloadErrors = function (data) {
  ...
  if (data.errors) {
    if (data.errors.length === 1) {
      err = data.errors[0]
      if (err.originalError && err.originalError.name !== 'GraphQLError') throw err.originalError
...
QueryExecutor._validateAST = function (documentAST, opts) {
  ...
  if (validationErrors.length) {
    ...
    var err = new GraphQL.GraphQLError('validation error')
  ...
  return new Promise(function (resolve, reject) {
    ...
    return Promise.resolve().then(function () {
      try {
        ...
        checkForPayloadErrors({ errors: [err] })
      }
    }).then(function (data) {
      // GraphQL.execute
